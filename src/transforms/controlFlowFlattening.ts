...
export default ({ Plugin }: PluginArg): PluginObject => {
  ...
  function cffMain(_path: NodePath) {
    ...
    const mainFnName = identifier("main");

    const scopeVar = identifier("scope");

    const statesVar = identifier("states");

    const runtimeVar = identifier("runtime");

    const stateVars = new Array(
      isDebug ? 1 : getRandomInteger(stateVarsRange[0], stateVarsRange[1]),
    )
      .fill("")
      .map((_, i) =>
    ...
    const didReturnVar = identifier("return");
      ...
      getObjectExpression(refreshLabel: string) {
        ...
        while (cursor) {
          ...
          if (parentScopeManager) {
            propertyMap[parentScopeManager.propertyName] = t.memberExpression(
              deepClone(scopeVar),
    ...
    function flattenIntoBasicBlocks(
      ...
      for (const index in body) {
        ...
        if (statement.isFunctionDeclaration()) {
          ...
          hoistedBasicBlock.body.unshift(
            t.variableDeclaration("var", [
              t.variableDeclarator(t.identifier(fnName), functionExpression),
            ]),
          );
        ...
        if (statement.isIfStatement() && flattenIfStatements) {
          ...
          if (
            ...
            const alternateLabel = alternate.node ? me.getPlaceholder() : null;
            ...
            currentBasicBlock.initializedScope = oldBasicBlock.scopeManager;
    ...
    for (const basicBlock of basicBlocks.values()) {
      ...
      let visitor: Visitor = {
        SequenceExpression: {
          enter(path) {
            const hasComment = (node: t.Node) =>
              node?.leadingComments?.some((p) =>
                p.value.includes("@js-confuser-assert"),
              );

            const asserted =
              hasComment(path.node) || hasComment(path.parentPath?.node);
            if (!asserted) {
              return;
            }
            ...
            if (exprs.length !== 2) {
              throw new Error(
                "Asserted sequence expressions must have exactly 2 expressions",
              );
            }

            const assertedValue = exprs[1].evaluate();
            if (!assertedValue.confident) {
              throw new Error(
                "Asserted sequence expression must be statically evaluable",
              );
            }
            ok(
              typeof assertedValue.value === "number",
              "Asserted sequence expression must evaluate to a number",
            );

            path.replaceWith(
              t.assignmentExpression("=", deepClone(runtimeVar), exprs[0].node),
            );
          },
        },
        ...
        StringLiteral: {
          exit(strPath) {
            ...
            const p = strPath.parentPath;
            if (
              p?.isCallExpression() &&
              t.isIdentifier(p.node.callee) &&
              p.node.callee.name === gotoFunctionName
            ) {
              return;
            }
        ...
        Identifier: {
          exit(path: NodePath<t.Identifier>) {
            ...
            let newName = scopeManager.getNewName(identifierName, path.node);
        ...
        ReturnStatement: {
          exit(path) {
            ...
            if (!functionParent || functionParent.get("body") !== blockPath)
              return;
        ...
        CallExpression: {
          exit(path) {
            if (
              ...
              const { stateValues: newStateValues, totalState: newTotalState } =
                jumpBlock;
              ...
              for (let i = 0; i < stateVars.length; i++) {
                ...
                if (!isDebug && addRelativeAssignments) {
                  ...
                  function mangledNumericLiteral(value: number) {
                    let stateVarIndex;
                    do {
                      stateVarIndex = getRandomInteger(0, stateVars.length);
                    } while (stateVarIndex === i && stateVars.length > 1);

                    const stateVarId = stateVars[stateVarIndex];
                    const stateVarValue = mutatingStateValues[stateVarIndex];
    ...
    for (const scopeManager of scopeToScopeManager.values()) {
      ...
      if (scopeManager.initializingBasicBlock.label === startLabel) continue;
    ...
    for (const block of blocks) {
      ...
      const sup = t.switchCase(lastTest, block.thisPath.node.body);
      const firstNode = block.thisPath.node.body[0];
      if (firstNode) {
    ...
    let switchStatement: t.Statement = t.inherits(
      t.labeledStatement(
        t.identifier(switchLabel),
        t.inherits(
          t.switchStatement(deepClone(discriminant), switchCases),
          programOrFunctionPath.node,
        ),
      ),
      programOrFunctionPath.node,
    );
    ...
    for (var [originalFnName, fnLabel, basicBlock, fn] of functionExpressions) {
      ...
      for (const parameterName of parametersNames) {
        if (parameterName === statesVar.name) {
          ...
          argumentsNodes.push(deepClone(runtimeVar));
    ...
    mainParameters.splice(
      ...
      1,
      t.assignmentPattern(deepClone(scopeVar), startProgramObjectExpression),
    );
    ...
    const resultVar = identifier("result");

    const isTopLevel = blockPath.isProgram();
    const allowReturns = !isTopLevel && blockPath.find((p) => p.isFunction());
